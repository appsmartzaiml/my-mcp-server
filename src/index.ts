...
interface Podcast {
    ...
    cat_name: string;
    total_stream: string;
    /** Exact ("3928") from the overall podcast search, rounded ("4K+") from the vector APIs. */
    total_play?: string;
    deeplink: string;
    cc_code: string;
}
...
interface PodcastView {
    ...
    category: string;
    language: string;
    /** Null when the API sent no play count. */
    playCount: number | null;
    /** Set when the API already rounded the count ("4K+"); shown as-is. */
    playsLabel: string;
}
...
function absoluteUrl(baseUrl: string, pathOrUrl: string | undefined): string {
    if (!pathOrUrl) return "";
...
function podcastPlayCount(value: string | undefined): number | null {
...
function buildRadioFmWidgetHtml(): string {
    return `
<!doctype html>
<html lang="en">
<head>
